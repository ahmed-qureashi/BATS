...
// BATS Escrow Contract ABI - Latest version with escrow functionality
export const BATS_ABI = [
	{
		"inputs": [
			{
	...
	},
	{
		...
		"outputs": [
			...
			{
				"internalType": "enum BATS.EscrowStatus",
				"name": "status",
				"type": "uint8"
			},
			{
...
export const getBATSContractAddress = (blockchain) => {
...
export const BATS_CONTRACT_ADDRESS = getBATSContractAddress('base')
...
/**
 * Get BATS contract instance with proper validation
 ...
 */
export const getBATSContract = (blockchain, network, privateKey = null) => {
    try {
        const contractAddress = getBATSContractAddress(blockchain)
        
        // Check if contract is deployed
        if (contractAddress === '0x0000000000000000000000000000000000000000') {
            throw new Error(`BATS contract not deployed on ${blockchain}`)
        }
        ...
        if (privateKey) {
            ...
            contractInstance = new ethers.Contract(contractAddress, BATS_ABI, wallet)
        } else {
            contractInstance = new ethers.Contract(contractAddress, BATS_ABI, provider)
        }
        ...
        console.error("Error creating BATS contract instance:", error)
        throw error
    }
}
...
export const validateContractConnectivity = async (blockchain, network) => {
    try {
        ...
        const contract = getBATSContract(blockchain, network)
