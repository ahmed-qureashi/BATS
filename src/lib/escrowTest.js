// Test file for BATS Escrow functionality
...
export const testEscrowFlow = async (blockchain, network, senderPrivateKey, receiverAddress, amount) => {
  console.log('🧪 Starting BATS Escrow Test Flow...')
  
  try {
    // Check if contract is configured
    if (!BATS_CONTRACT_ADDRESS || BATS_CONTRACT_ADDRESS === "0xYourBATSContractAddress") {
      throw new Error('BATS contract address not configured. Please deploy the contract and update .env')
    }

    console.log('📋 Contract Address:', BATS_CONTRACT_ADDRESS)
...
export const testContractConnectivity = async (blockchain, network) => {
  console.log('🔌 Testing BATS contract connectivity...')
  ...
  try {
    ...
    return {
      success: true,
      escrowCount,
      contractAddress: BATS_CONTRACT_ADDRESS
    }
...
console.log('🧪 BATS Escrow Tests loaded! Use window.BATSTests to run tests.')