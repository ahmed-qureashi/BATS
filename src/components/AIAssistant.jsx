...
import { BATS_SYSTEM_PROMPT, getFallbackResponse } from '../lib/BATSSystemPrompt'
...
const AIAssistant = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      text: "Hello! 👋 I'm BATS's Smart Assistant. How can I help you today?",
      sender: 'bot',
      timestamp: new Date()
    }
  ])
  ...
  // BATS AI API configuration 
  const BATS_API_KEY = import.meta.env.VITE_BATS_API_KEY
  const BATS_API_URL = import.meta.env.VITE_BATS_URL
  ...
  // Initialize BATS AI
  useEffect(() => {
    console.log('BATS AI Assistant initialized successfully')
    setIsLoaded(true)
  }, [])
  ...
  // Send message to BATS AI backend
  const sendToBATSAI = async (message) => {
    try {
      console.log('Sending message to BATS AI:', message)
      
      if (!BATS_API_KEY || BATS_API_KEY === 'undefined') {
        throw new Error('BATS AI service not configured')
      }

      // Use the comprehensive BATS system prompt
      const response = await fetch(`${BATS_API_URL}?key=${BATS_API_KEY}`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`BATS AI error: ${errorData.error?.message || response.statusText}`)
      }
      
      const data = await response.json()
      console.log('BATS AI response:', data)
      ...
      throw new Error('No response from BATS AI')
      
    } catch (error) {
      console.log('Using BATS AI fallback responses:', error.message)
  ...
  const handleSendMessage = async () => {
    ...
    // Send to BATS AI and get response
    const botResponses = await sendToBATSAI(inputValue)
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="px-4 py-3 rounded-2xl rounded-bl-sm bg-gray-800 text-gray-400 text-sm">
                        <div className="flex gap-1 items-center">
                          <span>BATS Assistant is thinking</span>
                ...
                <div className={`flex-shrink-0 ${isMobile ? 'p-3' : 'p-4'} border-t border-gray-600 bg-gray-900`}>
                  <div className="flex gap-2 items-end">
                    <textarea
                      ...
                      placeholder="Ask me anything about BATS..."
                      className={`
